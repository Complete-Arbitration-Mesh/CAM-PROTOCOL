...
export interface RateLimitConfig {
  ...
  requestsPerMinuteByTier?: {     // Override by subscription tier
    community: number;
    professional: number;
    enterprise: number;
  };
  providerLimitsPerMinute?: {     // Per-provider limits
    openai: number;
    anthropic: number;
    google: number;
    azure: number;
...
export class RateLimiter {
  ...
  private checkProviderLimit(providerId: string, now: number): RateLimitResult {
    ...
    if (this.config.providerLimitsPerMinute) {
      const typedLimits = this.config.providerLimitsPerMinute as Record<string, number>;
      limit = typedLimits[providerType] || limit;
    }

    const window = this.getOrCreateWindow(this.providerWindows, providerId, now);
    const requestsInWindow = this.countRequestsInWindow(window, now);
    const remaining = Math.max(0, limit - requestsInWindow);
    const resetAt = new Date(window.windowStart + this.WINDOW_SIZE_MS);
   ...
   */
  private recordRequest(userId: string, providerId: string | undefined, now: number): void {
    // Record user request
   ...
   */
  private countRequestsInWindow(window: SlidingWindowEntry, now: number): number {
    const cutoff = now - this.WINDOW_SIZE_MS;
   ...
   */
  private calculateRetryAfter(window: SlidingWindowEntry, limit: number, now: number): number {
  ...
  getStats(): RateLimitStats {
    ...
    return {
      ...
      blockRate: this.totalRequests > 0
        ? Math.round((this.blockedRequests / this.totalRequests) * 1000) / 1000
        : 0,
      activeUsers: this.userWindows.size,
      activeProviders: this.providerWindows.size
    };
