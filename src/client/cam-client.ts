...
export class CAMClient {
  ...
  async routeRequest(request: AICoreRequest): Promise<AICoreResponse> {
    ...
    const response = await this.makeRequest<AICoreResponse>(
      "POST",
      "/v2/route",
      {
        ...request,
  ...
  async getOptimalProvider(
    ...
    return this.makeRequest<ProviderInfo>(
      "POST",
      "/v2/providers/optimal",
      requirements,
    );
  }
  ...
  async initiateCollaboration(
    ...
    const session = await this.makeRequest<CollaborationSession>(
      "POST",
      "/v2/collaboration/initiate",
      {
        ...request,
        timestamp: new Date().toISOString(),
      },
  ...
  async discoverAgents(
    ...
    return this.makeRequest<AgentInfo[]>(
      "POST",
      "/v2/collaboration/agents/discover",
      {
        capabilities,
        timestamp: new Date().toISOString(),
      },
  ...
  async decomposeTask(task: ComplexTask): Promise<TaskComponents[]> {
    return this.makeRequest<TaskComponents[]>(
      "POST",
      "/v2/collaboration/tasks/decompose",
      {
        task,
        timestamp: new Date().toISOString(),
      },
  ...
  async executeWorkflow(
    ...
    const result = await this.makeRequest<CollaborationResult>(
      "POST",
      "/v2/collaboration/workflows/execute",
      {
        workflow,
        timestamp: new Date().toISOString(),
      },
  ...
  async getCollaborationStatus(
    ...
    return this.makeRequest<CollaborationSession>(
      "GET",
      `/v2/collaboration/sessions/${sessionId}`,
    );
  }
