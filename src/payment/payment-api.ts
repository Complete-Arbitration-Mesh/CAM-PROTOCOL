...
export class PaymentAPI {
  ...
  async getCustomer(
    ...
    try {
      ...
      const subscription =
        await this.subscriptionManager.getSubscriptionInfo(id);
  ...
  async createSubscription(
    ...
    try {
      ...
      if (!customerId || !planType) {
        reply
          .code(400)
          .send({ error: "Customer ID and plan type are required" });
        return;
      }
  ...
  async updateSubscription(
    ...
    try {
      ...
      if (!id || !planType) {
        reply
          .code(400)
          .send({ error: "Subscription ID and plan type are required" });
        return;
      }
  ...
  async createCheckoutSession(
    ...
    try {
      const { customerId, planType, successUrl, cancelUrl } =
        request.body as any;

      if (!customerId || !planType || !successUrl || !cancelUrl) {
        reply.code(400).send({
          error:
            "Customer ID, plan type, success URL, and cancel URL are required",
        });
        return;
      }
    ...
    try {
      ...
      switch (event.type) {
        ...
        case "invoice.payment_failed":
          this.logger.info("Invoice payment failed", {
            invoiceId: event.data.object.id,
          });
