...
export class SubscriptionManager {
  ...
  private readonly tierFeatures: Record<
    SubscriptionTier,
  ...
  async getSubscriptionInfo(
    ...
    try {
      ...
      if (!customer.subscriptions || customer.subscriptions.data.length === 0) {
        this.logger.info("No active subscription found for customer", {
          customerId,
        });
        return null;
      }
      ...
      if (!subscription) {
        throw new CAMError(
          "Subscription not found for customer: " + customerId,
      ...
      throw new CAMError(
        "Subscription error: Failed to retrieve subscription information",
  ...
  async upgradeSubscription(
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      throw new CAMError(
        "Subscription error: Failed to upgrade subscription",
        "SUBSCRIPTION_UPGRADE_ERROR",
        { details: { subscriptionId, newTier, errorMessage } },
      );
    }
  }
  ...
  async cancelSubscription(subscriptionId: string): Promise<void> {
    try {
      ...
      throw new CAMError(
        ...
        { details: { subscriptionId, errorMessage } },
      );
    }
  }
  ...
  async createCheckoutSession(
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      throw new CAMError(
        "Subscription error: Failed to create checkout session",
        "CHECKOUT_SESSION_ERROR",
        { details: { customerId, tier, errorMessage } },
      );
    }
  }
  ...
  async getUsageStats(customerId: string): Promise<{
    ...
    this.logger.warn("StateManager not configured, returning zero usage", {
      customerId,
    });
    return {
