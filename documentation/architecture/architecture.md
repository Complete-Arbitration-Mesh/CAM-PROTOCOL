# CAM Architecture Overview

## 1. Overview

The Cognitive Arbitration Mesh (CAM) is a platform that provides intelligent, policy-governed routing and failover between AI models. It serves as an infrastructure layer between applications and AI providers, enabling reliability, cost control, performance optimization, and compliance enforcement.

CAM follows a federated governance model and consists of core components (Apache 2.0 licensed) and enterprise components (Commons Clause licensed). The system is built using modern web technologies and follows a client-server architecture.

## 2. System Architecture

CAM follows a multi-tier architecture:

1. **Frontend**: React-based UI with Radix UI components and Tailwind CSS for styling
2. **Backend**: Node.js/Express.js server handling API requests and business logic
3. **Database**: PostgreSQL database accessed via Drizzle ORM
4. **Policy Enforcement**: Open Policy Agent (OPA) integration for policy decisions
5. **Performance Acceleration**: Optional eBPF acceleration for Enterprise Edition

The architecture is designed to be modular, with clear separation between the UI, business logic, and data access layers. The system can be deployed in various configurations, from single-instance development setups to multi-tenant enterprise deployments with namespace isolation.

## 3. Key Components

### 3.1. Frontend (Client)

- **Framework**: React with TypeScript
- **UI Components**: Radix UI primitives with a custom design system
- **Styling**: Tailwind CSS with a custom configuration
- **State Management**: React Query for server state, React hooks for local state
- **Build Tool**: Vite for fast HMR and build optimization
- **Developer Tools**:
  - Interactive Workflow Visualization Tool for workflow diagrams and simulation
  - Cost Calculator Widget for usage and pricing estimates
  - Code Snippet Generator for integration assistance
  - Debugging Dashboard with tier-based diagnostic capabilities

The frontend provides dashboards for monitoring system usage, configuration for model arbitration policies, administrative tools, and developer-friendly features for easier integration and troubleshooting.

### 3.2. Backend (Server)

- **Framework**: Express.js running on Node.js
- **API Style**: RESTful endpoints with JSON payloads
- **Authentication**: JWT-based authentication with Ed25519 signatures
- **Rate Limiting**: Express-based rate limiting for API protection

The server implements the core arbitration logic, policy enforcement, user management, and integration with AI providers. It exposes APIs for the frontend to consume.

### 3.3. Database

- **Type**: PostgreSQL (via NeonDB serverless connector)
- **ORM**: Drizzle ORM for type-safe database access
- **Schema**: User accounts, licenses, clusters, audit logs, and telemetry data
- **Migration**: Drizzle Kit for schema migrations

The database stores all persistent data, including user information, license details, and operational metrics.

### 3.4. Open Arbitration Specification (OASpec)

- **Format**: Protocol Buffers and JSON Schema
- **Version**: Currently v0.1
- **Purpose**: Standardizes the interface for AI model arbitration

OASpec is an open standard for defining how arbitration decisions are made between different AI models, providing a vendor-neutral specification.

### 3.5. Licensing and Policy Enforcement

- **License Verification**: Ed25519 cryptographic signatures
- **Policy Engine**: Open Policy Agent (OPA) integration
- **FIPS Compliance**: Optional FIPS 140-2 compliance mode for Enterprise Edition

The system enforces license constraints and security policies using cryptographic verification and policy-as-code approaches.

### 3.6. SDK and External Integrations

- **Lightweight SDK Variants**: Client libraries for JavaScript, TypeScript, Python, and Go
- **Stripe Integration**: For subscription management and payment processing
- **GitHub OAuth**: For user authentication
- **Local Development Environment**: Tools for local testing and development

These components allow for easier integration with external systems and services.

## 4. Data Flow

1. **User Authentication**:
   - Users authenticate via GitHub OAuth or email/password
   - Server validates credentials and issues a JWT token
   - Frontend stores the token for subsequent API calls

2. **Arbitration Process**:
   - Client applications send arbitration requests to the server
   - Server evaluates policies and model availability
   - Arbitration decision is made based on cost, latency, and other constraints
   - Selected model handles the request, with automatic failover if needed

3. **License Management**:
   - Licenses are created on the server with Ed25519 signatures
   - Client applications validate licenses locally and periodically check with the server
   - License constraints (cluster limits, feature access) are enforced

4. **Telemetry and Monitoring**:
   - System collects performance metrics and usage statistics
   - Data is processed and stored in the database
   - Dashboard visualizes metrics for users

## 5. External Dependencies

### 5.1. Required Dependencies

- **Database**: PostgreSQL (via NeonDB serverless)
- **Payment Processing**: Stripe for subscription management
- **Authentication**: GitHub OAuth for social login

### 5.2. Optional Dependencies

- **Monitoring**: Prometheus and Grafana dashboards
- **Policy Enforcement**: Open Policy Agent (OPA)
- **Performance**: eBPF for fast path acceleration (Enterprise Edition)

## 6. Deployment Strategy

CAM supports multiple deployment models:

### 6.1. Development/Testing

- Local development with Node.js and Vite
- Docker-based development environment
- Playwright for end-to-end testing
- Accessibility testing with Axe

### 6.2. Production Deployment

- **Standard Deployment**: Node.js application with PostgreSQL database
- **Kubernetes Deployment**: Helm charts for SMB and Enterprise editions
- **Multi-tenant Deployment**: Namespace isolation for Enterprise Edition

### 6.3. Scaling and High Availability

- Horizontal scaling via Kubernetes
- StatefulSet for stable persistence and identity
- Pod Disruption Budget for high availability
- Custom metrics-based autoscaling

## 7. Security Considerations

- **Authentication**: JWT-based with Ed25519 signatures
- **Authorization**: Role-based access control
- **Data Protection**: PII masking through OPA policies
- **Cryptography**: Optional FIPS 140-2 compliance mode
- **Network Security**: Kubernetes NetworkPolicies for isolation

## 8. Extensibility

The system is designed to be extensible in several ways:

- **Custom Policies**: Via OPA policy bundles
- **Model Integrations**: Through the Open Arbitration Specification
- **UI Customization**: Through theme configuration
- **Deployment Flexibility**: Through Helm value overrides

This architecture provides a balance of performance, security, and flexibility, allowing CAM to serve as a reliable arbitration layer for AI model selection and failover.